/* FONCTIONNALITÉS */ /* 1 - Retrouver un client */
DROP FUNCTION IF EXISTS recherche_client;


CREATE OR REPLACE FUNCTION recherche_client(nomClient VARCHAR, prenomClient VARCHAR, dateNaissanceClient DATE) RETURNS TABLE (idpers INTEGER, nomCl VARCHAR, prenomCl VARCHAR, dateNaissanceCl DATE, mailCl VARCHAR, numTelephoneCl VARCHAR, campingCl ECamping,
                                                                                                                                                                                                                                             statutCl EStatutClient,
                                                                                                                                                                                                                                             tailleCl INTEGER, poidsCl FLOAT, preferenceContactCl EPreferenceContact,
                                                                                                                                                                                                                                                                              idCertificatCl INTEGER) AS $$
...
CREATE OR REPLACE PROCEDURE creer_client(nom VARCHAR, prenom VARCHAR, dateNaissance DATE, mail VARCHAR, numTelephone VARCHAR, camping ECamping, statut EStatutClient, poids FLOAT, taille INTEGER, preferenceContact EPreferenceContact) AS $BODY$
BEGIN
...
CREATE OR REPLACE FUNCTION recherche_moniteur(nomClient VARCHAR, prenomClient VARCHAR, dateNaissanceClient DATE) RETURNS TABLE (idpers INTEGER, nomMo VARCHAR, prenomMo VARCHAR, dateNaissanceMo DATE, mailMo VARCHAR, numTelephoneMo VARCHAR) AS $$
...
CREATE OR REPLACE FUNCTION ConsulterListeEmploye() RETURNS TABLE ( Nom VARCHAR(30),
                                                                       Prenom VARCHAR(30),
                                                                              Mail VARCHAR(50),
                                                                                   NumTelephone VARCHAR(16),
                                                                                                DateNaissance DATE, TypeEmploye EtypeEmploye) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ce.nom,
...
CREATE OR REPLACE FUNCTION AfficherProfilEmploye(Nom VARCHAR(30), Prenom VARCHAR(30)) RETURNS TABLE ( nomEmploye VARCHAR(30),
                                                                                                                 prenomEmploye VARCHAR(30),
                                                                                                                               dateNaissanceEmploye DATE, mail VARCHAR(50),
                                                                                                                                                               numTelephone VARCHAR(30),
                                                                                                                                                                            TypeEmploye EtypeEmploye) AS $$
BEGIN
    RETURN QUERY
    SELECT
        Ce.nom,
...
/* 7 - Retrouver un employé */ --SELECT enum_range(null::ETypeEmploye); -- "{Propriétaire,Moniteur,""Garçon de Plage""}"

DROP FUNCTION IF EXISTS f_rechercher_employe;


CREATE OR REPLACE FUNCTION f_rechercher_employe(roleEmploye VARCHAR, nomEmploye VARCHAR, prenomEmploye VARCHAR, dateNaissanceEmploye DATE, mailEmploye VARCHAR, numTelEmploye VARCHAR) RETURNS TABLE (idEmp INTEGER, nomUtilEmp VARCHAR, nomEmp VARCHAR, prenomEmp VARCHAR, mailEmp VARCHAR, numTelEmp VARCHAR) AS $$
...
--SELECT f_rechercher_employe('Moniteur', 'BOND', 'James', '1996-08-04', 'jbond@lesabeilles.fr', null); -- test
 /* 8- Créer un propriétaire */ --SELECT * FROM CompteEmploye WHERE TypeEmploye = 'Propriétaire';

DROP FUNCTION IF EXISTS f_creer_proprietaire;


CREATE OR REPLACE FUNCTION f_creer_proprietaire(nomUtilisateur VARCHAR, motDePasse VARCHAR, nom VARCHAR, prenom VARCHAR, dateNaissance DATE, mail VARCHAR, numTelephone VARCHAR) RETURNS int AS $BODY$
DECLARE
...
/* Créer un profil moniteur (Propriétaire) */ --SELECT * FROM CompteEmploye WHERE TypeEmploye = 'Moniteur';
-- la FK diplôme est insérée au moment de créer le diplôme

DROP FUNCTION IF EXISTS f_creer_moniteur;


CREATE OR REPLACE FUNCTION f_creer_moniteur(nomUtilisateur VARCHAR, motdepasse VARCHAR, nom VARCHAR, prenom VARCHAR, dateNaissance DATE, mail VARCHAR, numTelephone VARCHAR) RETURNS int AS $BODY$
DECLARE
	nouvIdMoniteur int;
	nomUtil VARCHAR;
	mdp VARCHAR;
...
CREATE OR REPLACE PROCEDURE p_supprimer_employe(roleEmploye VARCHAR, nomEmploye VARCHAR, prenomEmploye VARCHAR, dateNaissanceEmploye DATE, mailEmploye VARCHAR, numTelEmploye VARCHAR) AS $BODY$
DECLARE
    idEmploye INTEGER;
	nomUtil VARCHAR;
BEGIN
...
CREATE OR REPLACE PROCEDURE p_creer_diplome(dateObtention DATE, LienDocumentPDF VARCHAR, IdMoniteur int) AS $BODY$
DECLARE
	nouvIdDiplome int;
BEGIN
...
WHERE t.relname ='diplome'
    OR t.relname = 'compteemploye';
...
CREATE OR REPLACE FUNCTION verification_utilisateur(identifiant VARCHAR, mdp VARCHAR) RETURNS BOOLEAN AS $$
DECLARE
    mdpcrypte VARCHAR;
BEGIN
...
CREATE OR REPLACE FUNCTION fetch_nom_moniteur() RETURNS TABLE (id_moniteur INT, nom_moniteur VARCHAR, prenom_moniteur VARCHAR, date_moniteur DATE) AS $$
...
CREATE OR REPLACE FUNCTION fetch_role_utilisateur(identifiant VARCHAR, mdp VARCHAR) RETURNS etypeemploye AS $$
DECLARE
	mdpcrypte VARCHAR;
BEGIN
    SELECT INTO mdpcrypte motdepasse FROM informations_connexion WHERE nomutilisateur=$1;
...
CREATE OR REPLACE FUNCTION verification_moniteur_disponible(idMoniteur INT, dateHeureCours TIMESTAMP) RETURNS BOOLEAN AS $$
...
CREATE OR REPLACE FUNCTION consulter_cours_voile() RETURNS TABLE ( IdCours int, DateHeure timestamp,
                                                                                          ...
                                                                                          NomMoniteur text, EtatCours Varchar(30)) AS $$
BEGIN
    RETURN QUERY
    SELECT
		cpv.IdCours,
...
/* 22 - Annuler un cours */ --SELECT * FROM CoursPlancheVoile;
--SELECT enum_range(null::EEtatCours); --"{Prévu,"En cours",Réalisé,Annulé}"

DROP FUNCTION IF EXISTS f_annuler_cours;


CREATE OR REPLACE FUNCTION f_annuler_cours(idCoursAnnule int) RETURNS int AS $$
...
CREATE OR REPLACE FUNCTION consulter_cours_voile_pour_inscription() RETURNS TABLE ( idCours INT, dateheure TIMESTAMP,
                                                                                                           niveau EStatutClient,
                                                                                                           nommoniteur text, nbplacesrestantes bigint) AS $$
BEGIN
    RETURN QUERY
    SELECT
        cpv.IdCours,
...
CREATE OR REPLACE PROCEDURE modifier_profil_client( idCli INT, nomClient VARCHAR, prenomClient VARCHAR, dateNaissanceClient DATE, mailClient VARCHAR, telClient VARCHAR, prefContactClient EPreferenceContact, campingClient ECamping, tailleClient INT, poidsClient FLOAT, statutClient EStatutClient) AS $BODY$
BEGIN
    UPDATE Client
    SET
        nom = nomClient,
...
CREATE OR REPLACE FUNCTION f_rechercher_standuppaddle(dateLoc TIMESTAMP, dureeLoc INTERVAL) RETURNS TABLE ( idMatos INTEGER, nomMateriel VARCHAR(30),
...
CREATE OR REPLACE FUNCTION f_rechercher_planchevoile(dateLoc timestamp, dureeLoc interval, capaciteFlot ecapaciteflotteur, tailVoile etaillevoile) RETURNS TABLE (idFloteur INTEGER, idPiedMat INTEGER, idDeVoile INTEGER) AS $$
DECLARE
	minTime timestamp;
	maxTime timestamp;
	idFloteur INTEGER;
...
CREATE OR REPLACE FUNCTION creer_garcon(nomUtilisateur VARCHAR, motdepasse VARCHAR, nom VARCHAR, prenom VARCHAR, dateNaissance DATE, mail VARCHAR, numTelephone VARCHAR) RETURNS int AS $BODY$
DECLARE
    nouvIdGarcon int;
    nomUtil VARCHAR;
    mdp VARCHAR;
...
CREATE OR REPLACE PROCEDURE acheter_forfait(idClientFor INTEGER, idForfait INTEGER, typePaiement EMoyenPaiement) AS $BODY$
DECLARE
	now timestamp;
	idPaie INTEGER;
	annee INTEGER;
...
BEGIN
	...
	IF(possede_remise(idClientFor)) THEN 
		montantForfait := montantForfait * 0.9;
	END IF;
...
CREATE OR REPLACE FUNCTION possede_remise(idPers INTEGER) RETURNS BOOLEAN AS $BODY$
BEGIN
...
BEGIN
    ...
    RETURN QUERY SELECT DISTINCT
        c.idCatamaran as idMatos,
        m.nomMateriel,
        c.idPrixMateriel as idPrixMatos,
        m.prixHeure,
        m.prixHeureSupp,
        m.prixDemiHeure,
        c.statut
    FROM
        Catamaran c
    JOIN
        PrixMateriel m ON c.idPrixMateriel = m.idPrixMateriel
    LEFT JOIN
        v_stock_materiel_raw v ON v.IdMatos = c.idCatamaran
    WHERE
        c.statut = 'Fonctionnel'
        AND NOT EXISTS (
            SELECT 1
            FROM
                Location t_loc
            WHERE
                t_loc.idCatamaran = c.idCatamaran
                AND t_loc.etatlocation = 'En cours'
                AND (
                    (t_loc.dateheurelocation BETWEEN minTime AND maxTime)
                    OR ((t_loc.dateheurelocation + t_loc.duree) BETWEEN minTime AND maxTime)
...
CREATE OR REPLACE FUNCTION f_rechercher_pedalo(dateLoc TIMESTAMP, dureeLoc INTERVAL) RETURNS TABLE ( idMatos INTEGER, nomMateriel VARCHAR(30),
                                                                                                                                  idPrixMatos INTEGER, prixHeure FLOAT, prixHeureSupp FLOAT, prixDemiHeure FLOAT, statut EStatutMateriel) AS $$
DECLARE
    minTime TIMESTAMP;
    maxTime TIMESTAMP;
BEGIN
    ...
    LEFT JOIN
        v_stock_materiel_raw c ON c.IdMatos = p.idpedalo
    WHERE
...
CREATE OR REPLACE FUNCTION creer_planche_voile(p_idFloteur INTEGER, p_idPiedMat INTEGER, p_idDeVoile INTEGER) RETURNS TABLE (idPlanche INTEGER) AS $BODY$
BEGIN
